(function () {
    ...
    const state = {
        profile: {
            ...
            role: "Điều phối viên",
            email: "dispatcher@carrental.vn",
            phone: "0901 234 567",
            code: "DP-001",
            department: "Điều phối vận hành",
            joined: "15/08/2026"
        },
        notifications: {
            bookingNew: true,
    ...
    function toast(msg) {
        // Layout renders #mkGlobalToast after @section Scripts — resolve lazily.
    ...
    function fillProfile() {
        ...
        els.dept.value = p.department;
        els.joined.value = p.joined;
        els.displayName.textContent = p.fullName;
        els.codeLabel.textContent = p.code;
        els.avatar.textContent = initials(p.fullName);
        if (els.profileError) {
    ...
    els.appearanceSave?.addEventListener("click", () => {
        ...
        if (theme === "dark") {
            toast("Đã chọn chế độ tối (mô phỏng) — giao diện vẫn giữ sáng.");
        } else {
            toast("Đã lưu tùy chọn giao diện");
        }
    });
    ...
    els.passwordForm?.addEventListener("submit", (e) => {
        ...
        if (neu.length < 8) {
            els.pwdError.hidden = false;
            els.pwdError.textContent = "Mật khẩu mới tối thiểu 8 ký tự.";
            return;
        }
        if (neu !== conf) {
            els.pwdError.hidden = false;
        ...
        toast("Đã cập nhật mật khẩu (mô phỏng)");
    });
