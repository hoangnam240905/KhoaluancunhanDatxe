(() => {
    ...
    const form = root.querySelector("[data-rv-form-el]");
    const ratingInput = root.querySelector("[data-rv-rating-input]");
    let rating = 0;
    ...
    const submitButtons = root.querySelectorAll("[data-rv-submit]");
    ...
    const syncScore = (value) => {
        ...
        if (form?.dataset.rvBusy === "1") return;
    ...
    form?.addEventListener("submit", (e) => {
        ...
        if (required() && !text) {
            e.preventDefault();
            showError("Vui lòng nhập nhận xét khi đánh giá từ 1 đến 3 sao.");
            return;
        }
    ...
    const initial = Number(ratingInput?.value || 0);
