...
public class CarRentalApiClient(HttpClient http, AuthSession auth)
{
    ...
    public async Task<(List<IncidentResponse> Data, string? Error)> GetDispatchIncidentsAsync(
        int? bookingId = null, string? status = null)
    {
        var parts = new List<string>();
        ...
        var url = parts.Count == 0 ? "/api/dispatch/incidents" : "/api/dispatch/incidents?" + string.Join("&", parts);
        using var request = CreateRequest(HttpMethod.Get, url);
        var response = await http.SendAsync(request);
        if (!response.IsSuccessStatusCode) return ([], await GetErrorAsync(response));
    ...
    public async Task<(List<VehicleInspectionResponse> Data, string? Error)> GetDispatchInspectionsAsync(int? bookingId = null)
    {
        var url = bookingId is int bid
            ? $"/api/dispatch/inspections?bookingId={bid}"
    ...
    {
        // null/empty => all drivers (GET /api/drivers); default remains Available for callers that omit status.
        var url = string.IsNullOrWhiteSpace(status)
            ? "/api/drivers"
            : $"/api/drivers?status={Uri.EscapeDataString(status)}";
        using var request = CreateRequest(HttpMethod.Get, url);
        var response = await http.SendAsync(request);
    ...
    {
        var (ok, error, _) = await ChangePasswordWithStatusAsync(oldPassword, newPassword);
        return (ok, error);
    }
